defmodule Timber.Plug do
  ...
  # Plug.RequestId module on the connection which sets a request ID only if one
  # hasn't already been set. If the request ID is set by a service prior to
  # Plug, it will be present as a request header. If Plug.RequestId generates a
  # request ID, that request ID is only present in the response headers. The
  # request headers should always take precedent in this function, though.
  #
  # This function will return either a single element list containing a
  # two-element tuple of the form:
  #
  ...
  # future processing.
  #
  # Note: Plug.RequestId will change an existing request ID if it doesn't think
  # the request ID is valid. See
  ...
  def get_request_id(conn, header_name) do
    ...
    handle_request_id(headers)
  end

  # Helper function to take the result of the header retrieval function and
  # change it to the desired response format for get_request_id/2
